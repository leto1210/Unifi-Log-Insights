...
The implementation now lives in the foundational `response_cache` module so the
whole API shares one cache with a single set of semantics (args-keyed,
deep-copied on read, single uvicorn worker → effectively shared). This module
re-exports it so the existing `from routes._response_cache import ttl_cache` /
`clear_cache` sites keep working.
...
from response_cache import _cache, clear_cache, ttl_cache
